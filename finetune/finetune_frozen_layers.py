...
# We froze some layers in the model which is detailed below:
# class Whisper(nn.Module):
#     def __init__(self, dims: ModelDimensions):
#         super().__init__()
#         self.dims = dims
            
#         # Freeze initial layers of decoder, only allow training final layers
#         n_trainable_layers = 2  # Adjust this fraction as needed
#         for i, block in enumerate(self.decoder.blocks):
#             # Freeze all layers except the last n_trainable_layers
#             if i < (self.dims.n_text_layer - n_trainable_layers):
#                 for param in block.parameters():
#                     param.requires_grad = False

#         for i, block in enumerate(self.encoder.blocks):
#             # Freeze all layers except the last n_trainable_layers
#             if i < (self.dims.n_audio_layer - n_trainable_layers):
#                 for param in block.parameters():
#                     param.requires_grad = False
                    
#         # Optionally, keep the final layer norm and token embedding trainable
#         for param in self.decoder.ln.parameters():
#             param.requires_grad = True
#         for param in self.decoder.token_embedding.parameters():
#             param.requires_grad = True
